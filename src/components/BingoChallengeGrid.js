...
export default class BingoChallengeGrid extends React.Component {
  constructor(props) {
    ...
    this.bingoChallengeGridTemplate =
      props.challengeBingo.bingoChallengeGridTemplate;
  }

  componentDidMount() {
    const freestyleSpace = _.find(
      this.taskCollection,
      space => space.category === "Freestyle"
    );
  ...
  arrangeBingoSpaces() {
    ...
    if (bannedBingoSpaces.length > 0) {
      bannedBingoSpaces.forEach(bannedSpace => {
        const indexOfBannedInClean = cleanBingoSpaces.findIndex(
          space => space.task === bannedSpace.task
        );
    ...
    this.bingoChallengeGridTemplate.categories.forEach((category, i) => {
      ...
      const matchedSpaceIndex = _.findIndex(
        shuffledBingoSpaceOptions,
        space => {
      ...
      if (matchedSpaceIndex > -1) {
        newlyArrangedBingoSpaces[i] =
          shuffledBingoSpaceOptions[matchedSpaceIndex];
        // center logic
        if (i === CENTER_INDEX) {
  ...
  unpinBingoSpace(i) {
    this.setState(state => {
      ...
      pinnedBingoSpaceIndexes = pinnedBingoSpaceIndexes.filter(
        index => index !== i
      );
  ...
  banBingoSpace(bannedBingoSpace, i) {
    this.setState(state => {
      ...
      let bingoSpacesWithoutBannedSpaces = cleanBingoSpaces.filter(
        cleanSpace => {
          // remove current space
          if (cleanSpace.id === bannedBingoSpace.id) {
            return false;
          }
          return !bannedBingoSpaces.find(
            bannedSpace => bannedSpace.id === cleanSpace.id
          );
        }
      );
      ...
      let newBingoSpace = bingoSpacesWithoutBannedSpaces.find(space => {
        return (
          ...
          bingoSpacesWithoutBannedSpaces.find(
            currentlyPlacedSpace => currentlyPlacedSpace.id !== space.id
          )
        );
      });
      ...
      if (!newBingoSpace) {
        newBingoSpace =
          bingoSpacesWithoutBannedSpaces[
            _.random(0, bingoSpacesWithoutBannedSpaces - 1)
          ];
      }
      ...
      return {
        ...
        arrangedBingoSpaces,
        bannedBingoSpaces,
        maxBingoSpacesBanned:
          bingoSpacesWithoutBannedSpaces.length < BINGO_SPACE_COUNT + 1
      };
    });
  }
  ...
  Pin(i) {
    ...
    return (
      <span
        className={`fa-stack ${!isPinned ? "text-muted" : ""}`}
        style={{ verticalAlign: "top" }}
        onClick={() =>
          isPinned ? this.unpinBingoSpace(i) : this.pinBingoSpace(i)
        }
      >
        <i className={`${isPinned ? "fas" : "far"} fa-circle fa-stack-2x`} />
        <i
          className={`fas fa-thumbtack fa-stack-1x ${
            isPinned ? "fa-inverse" : ""
          }`}
        />
      </span>
  ...
  Ban(space, i) {
    ...
    return (
      <span
        ...
        onClick={() => this.banBingoSpace(space, i)}
      />
    );
  }
    ...
    return (
      <header className="label no-print" onClick={() => this.toggleEditable()}>
        <button
          type="button"
          className={`far fa-${
            isEditingFreestyleTask ? "save" : "edit"
          } btn btn-${isEditingFreestyleTask ? "success" : "warning"} btn-sm`}
        />
      </header>
    );
  }
  ...
  SpaceHeader(space, i) {
    return (
      <header className={"pin-and-ban d-print-none"}>
        <span className="pin">{this.Pin(i)}</span>
        <span className="ban">{this.Ban(space, i)}</span>
      </header>
    );
  }
  ...
  Space(space, i) {
    return (
      <div
        ...
        className={`text-center space ${space.category} ${space.isCenter &&
          "editable"}`}
      >
  ...
  CreateTaskList() {
    ...
    return (
      <ListGroup className={"challenge-list"}>
        {arrangedBingoSpaces.map((space, i) => (
          <ListGroup.Item>
            <div className="pin">{this.Pin(i)}</div>
            <div className="task">{space.task}</div>
            <div className="ban">{this.Ban(space, i)}</div>
          </ListGroup.Item>
        ))}
      </ListGroup>
    );
  ...
  BannedSpaces() {
    ...
    return (
      <div className="banned-spaces">
        <h3 className="banned-spaces__title mb-3">
          Banned Spaces
          <span className="ml-2 fas fa-ban text-danger" />
          <button
            className="btn btn-success float-right"
            onClick={() => this.clearBannedSpaces()}
          >
            Add All Back In
          </button>
        </h3>
  ...
  render() {
    return (
      <>
        <Row className="no-print">
          <Col>
            <button
              ...
              onClick={() => this.arrangeBingoSpaces()}
            >
              Shuffle
            </button>
          </Col>
        ...
        <div className="list mb-4 d-md-none">{this.CreateTaskList()}</div>
        <div className="d-print-none">{this.BannedSpaces()}</div>
      </>
    );
  }
}
