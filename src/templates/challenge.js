...
export default class Challenge extends React.Component {
  ...
  render() {
    ...
    return (
      <Layout
        footerScripts={["//assets.pinterest.com/js/pinit.js"]}
        className="challenge"
      >
        <h1>{props.title}</h1>
        ...
        <section
          className="description no-print"
        ...
        <section className="related-posts no-print">
          <Row>
            {props.relevantBlogPosts.map(post => {
              return (
                <Col className={"mb-4"} key={post.slug} sm={6} md={4}>
                  <Card>
                    <Card.Body>
        ...
        <footer className={"print-only text-center"}>
          Pinterest Board: {props.pinterestLink}
        </footer>
      </Layout>
    );
  }
